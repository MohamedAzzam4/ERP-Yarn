...
| DEC-039 | The MVP uses a light-only Modern Industrial design system built from shadcn/ui, Radix, Tailwind CSS v4, React Hook Form/Zod, stable TanStack Table, Recharts, and Lucide. | Dark mode and end-user theme editing are deferred; components use centralized semantic tokens. |
...
| DEC-059 | A GLM sandbox without an owner-controlled secret channel uses credentialless Git artifact handoff, never credentials pasted into chat. | After tests and secret scanning, GLM exports the phase commit as a verified Git bundle outside the repository. A trusted authenticated environment imports it onto current `main`, reruns required gates, and pushes the phase branch. The WP remains incomplete/publication-pending until remote branch verification succeeds. |

## Integrated Design-System Decisions

- Light theme only in MVP; dark mode deferred.
- Modern Industrial visual direction: structured, geometric, operational, professional, clear, and restrained.
