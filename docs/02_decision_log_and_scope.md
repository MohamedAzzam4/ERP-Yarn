...
| DEC-075 | Phase-branch checkpoint pushes are allowed and recommended after meaningful GLM work-package checkpoints. | Because GLM sandboxes can reset and lose unpushed work, coding agents should request authorization to push the active phase branch after a local implementation checkpoint, defect-fix checkpoint, or incomplete-but-valuable recovery checkpoint. A pushed phase branch is only a recoverable checkpoint/publication artifact; it does not mean the package is accepted, complete, validated, merge-ready, or authorized for `main`. `main` remains protected by full package gates, required hosted validation/preview smoke where applicable, and explicit owner merge authorization. |
| DEC-076 | Calm Enterprise remains the visual baseline; restrained glass accents are permitted only as secondary management-surface accents. | Glassmorphism must not become the primary ERP style. It is prohibited for Worker Task Mode, dense forms, tables, financial numbers, approval actions, warnings/errors, and any surface where contrast/readability would weaken. Any glass accent must pass WCAG 2.2 AA, use semantic tokens, have solid fallback, respect reduced motion, and avoid excessive blur/shadow. |
| DEC-077 | Reference-screen Arabic terminology uses the owner-provided client workbook `متابعة انتاج وبيع خيوط الغزل (3).xlsx` and any later client Excel/screenshots first, then the provisional terminology fixture in `docs/design/01_reference_screen_terms_and_fixtures.md`. | `PCD-UX-001` is resolved for Phase 1 reference screens. If a term is absent from provided client material and the fixture, coding agents must mark it `Unresolved / requires owner decision` rather than inventing business terminology. |
| DEC-078 | The canonical synthetic and prohibited-data fixture for the three Phase 1 reference screens is `reference-fixtures-v1` in `docs/design/01_reference_screen_terms_and_fixtures.md`. | `PCD-UX-004` is resolved for the Worker raw-material receipt, Accountant review queue and Owner dashboard reference screens. Fixtures remain synthetic, versioned and replaceable by a later owner-approved version. |
| DEC-079 | Reference-screen approval evidence is stored through `docs/design/reference_screen_approval.md` and versioned evidence under `docs/design/evidence/reference-screens/<version>/`. | `PCD-UX-002` is resolved as a mechanism only. No screen is approved until the approval record contains owner decision evidence. Later UI changes create a new approval version instead of rewriting history. |
| DEC-080 | The requester cannot approve their own high-risk request in MVP. | Owner and Accountant may approve where the transaction type permits, but neither may approve their own request. Workers cannot approve financial, accounting, stock-impacting, migration or high-risk operational transactions. Historical migration remains stricter: Owner and Accountant approvals require two distinct users. Emergency self-approval is deferred and not allowed in MVP. |
| DEC-082 | Sales discount residual allocation is bounded by per-line invariants and may span multiple lines when necessary. | Normal case: the rounding residual starts with the largest gross-revenue line (tie: lowest stable `line_no`) as under DEC-048. Safety fallback: if one line cannot absorb the signed residual without violating `line_allocated_discount_posted >= 0`, `line_allocated_discount_posted <= line_gross_revenue`, `line_net_revenue_posted >= 0`, or `line_net_revenue_posted <= line_gross_revenue`, the residual continues to the next deterministic line in the same priority order (largest gross, then lowest `line_no`). Multiple lines may have non-zero `rounding_adjustment` only when required to preserve these invariants. If residual cannot be fully reconciled while preserving invariants, the calculation is rejected and requires owner decision. This decision supersedes the older single-line-only residual wording in DEC-048 for sales discount allocation. |
| DEC-081 | Historical migration alias/master approval is per alias mapping group, not per imported row. Repeated occurrences belonging to the same reviewed alias mapping are approved once. One authorized Owner OR one authorized Accountant may approve that alias mapping. The authorized reviewer may both select the target canonical master and approve the mapping themselves; no separate requester-versus-approver segregation is required for alias/master mapping review. Alias/master mapping review is not a high-risk approval request for purposes of DEC-080, so DEC-080 requester-versus-approver self-approval restrictions do not apply to alias/master mapping confirmation. This is a narrowly scoped migration clarification; it does NOT weaken or supersede DEC-080 for high-risk approval requests, transactional approvals, posting approvals, or other workflows governed by DEC-080. Alias approval is separate from migration batch approval and does not replace the required Owner AND Accountant dual approval for historical commit (DEC-020, DEC-069). | The approved mapping applies to all rows in that mapping group unless specific rows are explicitly separated into reviewed exceptions or subgroups. Do not require repeated human approval for every occurrence of the same alias; preserve source-row provenance. Each explicit exception/subgroup requires its own mapping decision because the same source alias may actually represent different real entities. A group-level approval must never silently override an explicit exception. Bind an approved alias mapping to at least source/normalized alias identity, target master, approving actor, approval time, and mapping/version identity; a material mapping change invalidates dependent validation, reconciliation, review/acceptance evidence, and batch approvals as applicable. AI, fuzzy matching, similarity scores, or identical-looking text may suggest candidates but may never approve, create, merge, or silently select a canonical master. If no valid canonical master exists, the alias remains unresolved until the official master is created through the normal Master Data workflow. |
| DEC-083 | Official canonical master data must have a dedicated management creation workflow in the MVP. Operational forms and historical migration may reference existing masters or capture a safe temporary "other / not listed" value for review, but they must not silently create official customers, suppliers, factories, locations, items/product types, or other canonical masters. Official master creation must use the normal Master Data domain workflow and its existing permission and validation rules. | The management frontend must eventually provide user-facing creation forms/actions for supported master-data entities. If an unresolved historical alias has no correct master, the master is created through the normal Master Data workflow and the user then returns to the migration mapping. Do not implement a second or hidden master-creation mechanism inside migration. Do not invent new role permissions for creating masters; the exact roles allowed to create/update each master type must continue to follow the existing permission matrix/contracts. |
| DEC-084 | The current functional historical-migration UI is sufficient while the migration backend/domain workflow is completed. Advanced migration-review UX is deferred to a later frontend package. | Backend/database/domain behavior for mapping, validation, review, alias approval, reconciliation, submission, approval and commit must still be completed correctly now. The later frontend improvement may add grouped issue review, bulk decisions such as "apply to all matching records", row-level exceptions, easier missing-data review, grouped alias review, bulk resolution where safe, and a shortcut from migration review to the normal Master Data creation workflow. The general Master Data creation UI is also required before the MVP is considered functionally complete, but its polished frontend implementation does not need to block the current historical-migration backend work. |
| DEC-061 | MVP users normally have one active operational role, while the schema may remain capable of multiple role assignments for future or exceptional Owner-managed cases. | MVP seeds, UI and tests must not rely on multi-role users. If multiple roles exist, effective permissions are the union of allowed actions except where a stricter denial/field ceiling applies; Worker-family financial denial always wins. Multi-role assignment is Owner-only and audited. DEC-069 separately controls historical migration dual-approval identity. |
...
| DEC-064 | Ordinary MVP transfers support only accepted/sellable unblocked stock. | Blocked, needs-review, partially blocked, returned, discounted-return or otherwise risky classification stock cannot move through ordinary transfer. It must first receive approved disposition/correction that makes the transferable quantity explicitly sellable/unblocked or routes it through a later special workflow. |
| DEC-065 | MVP sale reservation supports only accepted/sellable stock. | Needs-review, blocked, discounted-return or other quality-risk stock must go through review/disposition before sale submission/reservation. No protected risk reservation flow is implemented in MVP. |
| DEC-066 | MVP user-facing payment methods are a small fixed list. | Seed/validate only `cash`, `bank_transfer`, `check`, `wallet_instapay`, and `other`. Additional method names are future configuration, not coding-agent invention. |
| DEC-067 | Raw purchase payable uses net accepted weight as the tonnage basis. | Payable is `net_accepted_kg / 1000 × price_per_ton`. Accountant may enter/confirm late price; Owner may approve/correct where required. Workers cannot enter, see or confirm purchase price/payable. |
| DEC-068 | Partial-return credit residual is corrected on the final return. | Each partial return uses original approved sale-line net unit value after allocated discount. The final effective return adjusts the rounding residual so cumulative posted credits equal, and never exceed, the original posted sale-line net value. |
| DEC-069 | Historical migration dual approval requires two distinct user identities. | One multi-role identity cannot satisfy both Owner and Accountant approval records for migration commit. |
...
- Restrained glass accents are permitted only as secondary management-surface accents under DEC-076; glassmorphism remains prohibited as the primary ERP style and is prohibited in Worker Task Mode.
...
- Reference-screen Arabic terminology follows DEC-077 and `docs/design/01_reference_screen_terms_and_fixtures.md`.
- Reference-screen synthetic/prohibited fixtures follow DEC-078 and `reference-fixtures-v1`.
- Reference-screen approval evidence follows DEC-079 and `docs/design/reference_screen_approval.md`.
...
Official canonical masters are created through the normal Master Data workflow (DEC-083). Operational forms and historical migration may reference existing masters or capture a safe temporary "other / not listed" value for review; those temporary values never create official masters silently. Historical migration must not implement a second or hidden master-creation mechanism, and AI/fuzzy matching may never create or approve official masters. User-facing management creation screens for supported master-data entities are required before MVP completion; their polished implementation may come in a later frontend package under DEC-084.
...
| PCD-INV-001 | Resolved by DEC-064. Ordinary transfer moves only accepted/sellable unblocked stock; risky/returned classifications require prior approved disposition/correction. | `WP-00-03B` may implement inventory/reservation structures without classification-allocation invention. |
| PCD-SALE-001 | Resolved by DEC-065. Sale submission/reservation is allowed only for accepted/sellable stock; quality-risk stock requires review/disposition first. | `WP-00-03B` and later sales packages may block risky reservations instead of inventing a protected review reservation flow. |
| PCD-PAY-001 | Resolved by DEC-066. MVP payment methods are `cash`, `bank_transfer`, `check`, `wallet_instapay`, and `other`. | `WP-00-03D` may seed constrained payment method keys. |
| PCD-RAW-001 | Resolved by DEC-067. Raw purchase payable uses net accepted kg; Accountant confirms late price; Owner can approve/correct where required; Workers cannot enter/see price. | Raw receipt schema/approval packages may implement priced paths without guessing the basis/authority. |
| PCD-RET-001 | Resolved by DEC-068. Final partial return adjusts rounding residual so cumulative credits equal but never exceed original posted sale-line net value. | Return schema/service packages may implement partial return credit posting. |
| PCD-MIG-001 | Resolved by DEC-069. Historical migration Owner and Accountant approvals must be two distinct user identities. | Historical migration schema/approval packages may enforce distinct approver identity. |
...
| PCD-UX-001 | Resolved by DEC-077. Use the owner-provided client workbook `متابعة انتاج وبيع خيوط الغزل (3).xlsx` and any later client Excel/screenshots first, then the provisional Arabic terminology fixture in `docs/design/01_reference_screen_terms_and_fixtures.md`. | Reference-screen packages may use the listed provisional Arabic terms without inventing new business labels. |
| PCD-UX-002 | Resolved by DEC-079 as a storage/sign-off mechanism. Approval evidence is recorded in `docs/design/reference_screen_approval.md`; actual screen approval still requires owner decision evidence. **Owner decision evidence recorded 2026-07-01 — reference-screens-v1 APPROVED (all three screens).** | `WP-01-08` may use the canonical approval record, but it cannot infer approval before evidence is recorded. **WP-01-08 gate PASSED 2026-07-01 — broad frontend expansion unblocked.** |
| PCD-UX-004 | Resolved by DEC-078. Use `reference-fixtures-v1` in `docs/design/01_reference_screen_terms_and_fixtures.md` for the three reference screens. | `WP-01-05`, `WP-01-06` and `WP-01-07` may build deterministic reference screens and prohibited-data tests. |
| PCD-APR-001 | Resolved by DEC-080. Requesters cannot approve their own high-risk requests; Owner and Accountant can approve allowed transaction types except their own requests; Workers cannot approve high-risk financial/stock/migration transactions. | Approval seed/transaction packages may enforce segregation of duties without guessing self-approval behavior. |
...
16. Historical alias/master approval is per alias mapping group, not per imported row; repeated occurrences of the same reviewed alias mapping are approved once (DEC-081).
17. One authorized Owner OR one authorized Accountant approves an alias mapping group, and the same reviewer may both select the target canonical master and approve the mapping; alias review does not require a second actor (DEC-081).
18. Alias/master mapping review is outside the DEC-080 requester-versus-approver restriction; DEC-080 remains unchanged for high-risk approval workflows, and the Owner + Accountant dual approval for historical batch commit is not replaced by alias approval (DEC-081).
19. The same source alias may map to different real entities; explicit reviewed exceptions or subgroups each require their own mapping decision, and a group-level approval must never silently override an explicit exception (DEC-081).
20. Official canonical masters are created only through the normal Master Data workflow; operational and migration flows never silently create official masters, and unresolved aliases block submission until an official master exists (DEC-083).
