...
Every implementation package runs on the current `phase/NN-name` branch. GLM may push only that branch, never `main`. Under DEC-075, a phase-branch push is allowed and recommended after a meaningful local checkpoint, defect-fix checkpoint, or incomplete-but-valuable recovery checkpoint so sandbox resets do not destroy work. Such a push is a checkpoint/publication artifact only; it does not mean the package is accepted, complete, validated, merge-ready, or authorized for `main`. Package completion does not authorize merge: the phase PR remains unmergeable until every phase gate and required preview/integration check passes and the owner explicitly authorizes that phase merge.
...
- **Implementation notes:** Decimal only; posted lines/residual/document total contract. Residual allocation follows DEC-048 (normal: largest gross line, tie: lowest line_no) with DEC-082 safety fallback (multi-line bounded distribution when single-line absorption would violate per-line invariants).
- **Tests:** zero/discount, tie/largest residual, pathological many-line bounded residual (DEC-082), precision, role fields, submission reservations.
