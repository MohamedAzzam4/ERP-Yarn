...
## Discovered Count: 57 paths

The canonical search now discovers 57 paths:
- 55 executable destructive test/QA harness files (Category A)
...
- 1 new Category A file: wp-07-04-r27-postgres-closure.test.ts (WP-07-04 r27 deterministic service-vs-service PG concurrency proofs)
- 1 new Category A file: wp-07-04-r28-inventory-shared.test.ts (WP-07-04 r28 LIVE-LIVE-SHARED-INVENTORY-SVC-DET proof)

## Category Counts

- Category A (executable destructive test/QA harness): 55
...
| 56 | src/server/services/__tests__/wp-07-04-r27-postgres-closure.test.ts | 86-95, 298-309 | DELETE payment_settlements, payments, account_entries, accounts, audit_logs (with trigger disable), idempotency_records, document_sequences, customers, users, tenants (run-scoped tenant cleanup) | A | Shared guard (checkDestructiveTestDbSafety) | WP-07-04 r27 DETERMINISTIC service-vs-service PG concurrency proofs (SETTLE-RACE-1-SVC/2A-SVC/2B-SVC/3-TARGET-SVC, LIVE-LIVE-SHARED-SUBLEDGER-SVC-DET). Uses run-scoped tenants (RUN_ID = randomUUID). Disables audit_logs_no_delete/no_update triggers temporarily for test cleanup, then re-enables. |
| 57 | src/server/services/__tests__/wp-07-04-r28-inventory-shared.test.ts | 91-99, 183-189 | DELETE inventory_balances, stock_movements, audit_logs (with trigger disable), idempotency_records, document_sequences, inventory_items, locations, users, tenants (run-scoped tenant cleanup) | A | Shared guard (checkDestructiveTestDbSafety) | WP-07-04 r28 LIVE-LIVE-SHARED-INVENTORY-SVC-DET proof — two real InventoryLedgerService.postRawReceipt commands acquire SHARED cutover lock simultaneously. Uses run-scoped tenants (RUN_ID = randomUUID). Disables audit_logs_no_delete/no_update triggers temporarily for test cleanup, then re-enables. |
